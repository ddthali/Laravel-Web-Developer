...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Aldrich&family=Manrope&family=Mitr:wght@300&family=Rubik&display=swap"
        rel="stylesheet">
                    ...
                    <div id="aboutcmg"
                        class="Content-light container p-4 mt-4 mb-5 rounded-3">
                                ...
                                @php
                                    ...
                                    $box_large = false;
                                    $medium = false;
                                    if(preg_match("/\d+\. /",$exp->w_detail)>0){
                                        $box_large = true;
                                        $string = $exp->w_detail;
                                        $details = preg_split('/\d+\. /',$string);
                                        if(empty($details[0])){
                                            array_shift($details);
                                        }
                                        if(count($details)<7){
                                ...
                                @endphp
                                <div @class(['box-outside-lg' => $box_large && !$medium,'box-os-medium' => $medium,'box-outside' => !$box_large && !$is_last && !$medium,'box-os-last' => $is_last,'rounded-3'])>
                                    <div class="box-inside">
                                        ...
                                        <p> <span class="topic-b">Project Name :</span>  {{ $exp->w_project }}<br>
                                            ...
                                            <span class="topic-b">Period :</span>  {{ $exp->w_period }}<br>
                                            <span class="topic-b">Location :</span>  {{ $exp->w_location }}<br>
                                            <span class="topic-b">TYPE of CONTRACTTOR :</span>  {{ $exp->w_toct }}<br>
                                            <span class="topic-b">CONTRACTOR VALUE :</span>  {{ $exp->w_cont }}<br>
                                            <span class="topic-b">Detail of Work :</span>
                                            @if ($box_large)
                                ...
                                <br>
                                <div class="row gx-0">
                                @foreach ($workimg->where('sub_cate',$exp->sub_cate) as $imgs)
                                <div class="col-md-6">
                                    <img class="img-fluid img-view cv-anim" src="{{  asset('storage/'.$imgs->path) }}" title="{{$imgs->path}}">
                                </div>
                            @endforeach
                                </div>
                                <br>
