...
<body>
    <div class="container mt-2">
        <div class="row">
            <div class="col-lg-12 text-center">
                <h2 style="font-family: 'Mitr',sans-serif;">บริษัทผู้จัดจำหน่ายทั้งหมด</h2>
            </div>
            <div class="font-thai mb-2">
                <a href="{{ route('backend.vendor.create') }}" class="btn btn-success">เพิ่ม</a>
            </div>
            ...
            <table id="vendor" class="table table-bordered">
                <thead>
                    <tr>
                        ...
                        <td class="font-thai">ชื่อบริษัท</td>
                        <td class="font-thai">หมวดหมู่หลัก</td>
                        <td class="font-thai">หมวดหมู่รอง</td>
                    ...
                    <tr>
                        ...
                        <td class="font-thai">
                            <form id="delete-form-{{ $ven->v_id }}" action="{{ route('backend.vendor.destroy', $ven->v_id) }}" method="post">
                                <a href="{{ route('backend.vendor.edit', $ven->v_id) }}" class="btn btn-warning"><span class="font-thai">แก้ไข</span></a>
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-danger" onclick="deleteRecord({{ $ven->v_id }},'{{ $ven->v_client }}')"><span class="font-thai">ลบ</span></button>
                            </form>
                        </td>
                    </tr>
                    @endforeach
    ...
    <script>
      ...
      Swal.fire({
        title: 'คุณแน่ใจว่าจะลบบริษัทผู้จำหน่ายนี้หรือไม่?',
        text: `บริษัทนี้ '${cname}' จะหายไปเมื่อคุณลบ!`,
        icon: 'warning',
