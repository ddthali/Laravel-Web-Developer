...
<head>
    ...
    <title>Backend - Edit Slide</title>
...
<body>
    <div class="container mt-2">
        <div class="row">
            <div class="col-lg-12">
                <h2 class="font-thai">แก้ไขสไลด์</h2>
            </div>
            <div>
            ...
            <form action="{{ route('backend.gallery.update', $gallery->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select class="form-select" id="category" name="category">
                                @foreach ($cate as $cgry)
                                    <option class="font-thai" value="{{$cgry->id}}" {{ $gallery->img_cate == $cgry->id ? 'selected' : '' }}>{{$cgry->n_cate}}</option>
                                @endforeach
                              </select>
                              @error('category')
                        ...
                        <div class="form-group">
                            <label for="subcategory" class="font-thai">หมวดหมู่รอง</label>
                            <select class="form-select" id="subcategory" name="subcate">
                                @foreach ($wexp as $wxp)
                                    <option class="font-thai" value="{{$wxp->sc_id}}" {{ $gallery->sub_cate == $wxp->sc_id ? 'selected' : '' }}>{{$wxp->name}}</option>
                                @endforeach
                              </select>
                              @error('category')
                              <div class="alert alert-danger">
                        ...
                        <div class="form-group">
                            <label for="image" class="font-thai">อัพโหลดรูป</label>
                            <br>
