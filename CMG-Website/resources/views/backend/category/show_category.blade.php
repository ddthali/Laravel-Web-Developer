    ...
    <div class="container mt-2">
        <div class="row">
            <div class="col-lg-12 text-center">
                <h2 style="font-family: 'Mitr',sans-serif;">หมวดหมู่ทั้งหมด</h2>
            </div>
            <div>
                <a href="{{ route('backend.category.create') }}" class="btn btn-success">Create</a>
            </div>
            ...
            @endif
            <table id="catecade" class="table table-bordered">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="font-thai">{{ $cate->id }}</td>
                        <td class="font-thai">{{ $cate->n_cate }}</td>
                        <td class="font-thai">
                            <form id="delete-form-{{ $cate->id }}" action="{{ route('backend.category.destroy', $cate->id) }}" method="post">
                                <a href="{{ route('backend.category.edit', $cate->id) }}" class="btn btn-warning"><span class="font-thai">แก้ไข</span></a>
                                @csrf
                                @method('DELETE')
                                <button type="button" class="btn btn-danger" onclick="deleteRecord({{ $cate->id }},'{{ $cate->n_cate }}')"><span class="font-thai">ลบ</span></button>
                            </form>
                        </td>
                    </tr>
                    @endforeach
